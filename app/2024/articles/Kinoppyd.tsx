  ...
  return (
    <div id={'kinoppyd'}>
      <Page pageNumber={pageNumber}>
        ...
        <p>これは、神の雫という漫画の第二話で、天才ワイン評論家と呼ばれる人物が、銘柄が隠されたワインを口にして発した言葉です。銘柄を隠した状態でワインをテイスティングし、その性質を推量することををブラインドテイスティングといいますが、この漫画の天才評論家は香りとたったひとくちワインを口に含んだだけで、銘柄のみならずワインが作成された年数まで的中させます。では実際にブラインドテイスティングで、ワインの銘柄とヴィンテージ（ワインの制作された年）を言い当てることはできるのでしょうか？　その答えは、ほぼ不可能です。現実の競技としてのブラインドテイスティングでは、主に「ブドウの品種」「ワインの産地」「製造年数」を当てる競技です。もちろん、その三点はほぼ必須となるだけで、他にも大会によって様々な要素を推察することはありますが、銘柄と製造年数をピタリと当てることを前提とした大会はありません。なぜならば、ブラインドテイスティングで銘柄を的中させることはあまりにも難易度が高く、なにか制限が無い限りはまったく競技にならないからです。そのため、ワインの香りをひと嗅ぎして銘柄と製造年数を的中させるのは、あくまでフィクションの中の出来事で、現実には不可能です。</p>
